...
public class GodotApp: ObservableObject {
    ...
    public func emitMessage(_ message: VariantDictionary, from viewId: Int64? = nil) {
        runOnGodotThread { [weak self] in
            guard let self, let scene = self.hostScene() else { return }
            let payload = VariantDictionary(from: message)
            if let viewId {
                payload[BridgeRouting.viewIdKey] = Variant(viewId)
            }
            _ = scene.call(method: "host_receive_message", Variant(payload))
        }
    }
    ...
    private func hostScene() -> Node? {
        guard let instance, instance.isStarted() else { return nil }
        guard let sceneTree = Engine.getMainLoop() as? SceneTree else { return nil }
        return sceneTree.currentScene
    }

    private func pollHostMessages() {
        guard let scene = hostScene(), scene.hasMethod("host_poll_message") else { return }
        for _ in 0..<8 {
            guard
                let result = scene.call(method: "host_poll_message"),
                let message = VariantDictionary.fromVariant(result),
                !message.isEmpty()
            else {
                return
            }
