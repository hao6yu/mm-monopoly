...
public class UIGodotWindow: UIView {
    ...
    private func attach(
        ...
        if window.hasMethod(setNativeSurfaceMethod) {
            let windowNativeSurface = DisplayServerAppleEmbeddedBridge.createNativeSurface(
                layer: UInt(bitPattern: Unmanaged.passUnretained(windowLayer).toOpaque())
            )
            _ = window.call(method: setNativeSurfaceMethod, Variant(windowNativeSurface))
