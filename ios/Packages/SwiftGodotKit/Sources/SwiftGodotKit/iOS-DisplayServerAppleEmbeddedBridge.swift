...
enum DisplayServerAppleEmbeddedBridge {
    ...
    private static var nativeSurfaceClassName = StringName("RenderingNativeSurfaceApple")

    private static let methodSetNativeSurface = methodBind("set_native_surface", hash: 2894024005)
    private static let methodCreateNativeSurface = nativeSurfaceMethodBind("create", hash: 4233771684)
    ...
    private static func nativeSurfaceMethodBind(_ method: StaticString, hash: Int64) -> UnsafeRawPointer? {
        var methodName = FastStringName(method)
        return withUnsafePointer(to: &nativeSurfaceClassName.content) { classPtr in
            withUnsafePointer(to: &methodName.content) { methodPtr in
                gi.classdb_get_method_bind(classPtr, methodPtr, hash)
            }
        }
    ...
    static func createNativeSurface(layer: UInt) -> RefCounted? {
        guard let method = methodCreateNativeSurface else { return nil }
        var layerValue = Int64(bitPattern: UInt64(layer))
        var result = GodotNativeObjectPointer(bitPattern: 0)
        withUnsafePointer(to: &layerValue) { pArg0 in
            let args: [UnsafeRawPointer?] = [UnsafeRawPointer(pArg0)]
            args.withUnsafeBufferPointer { pArgs in
                gi.object_method_bind_ptrcall(method, nil, pArgs.baseAddress, &result)
            }
        }
